...
router.put("/orders/:order_id/cancel", userPassport, async (req, res) => {
  try {
    ...
    await query(updateSql, ["cancelled", orderId]);

    const orderDetailsSql =
      "SELECT productid, quantity FROM order_details WHERE order_id = ?";
    const orderDetails = await query(orderDetailsSql, [orderId]);
    ...
    for (const orderItem of orderDetails) {
      ...
      const updateProductStockSql =
        "UPDATE onlineproducts SET stock = stock + ? WHERE productid = ?";
      await query(updateProductStockSql, [quantity, productid]);
    }
